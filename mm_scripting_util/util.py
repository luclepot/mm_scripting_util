...
class _mm_base_util:
    ...
    def _tabulate_comparison_information(
        ...
        self.log.info("")
        self.log.info(
            "Printing comparison information for mg5 and augmented histograms..."
        )
        self.log.info(
            "This shows the ratios between the datasets, flagging values for which"
        )
        self.log.info(
            "this ratio is greater than the given threshold of {}.".format(threshold)
        )
        self.log.info("")
    ...
    def _scale_to_range(self, arr, _range):
        # normalkized to 0,1
        arr = (arr - np.min(arr)) / (np.max(arr) - np.min(arr))
        return (arr) * (_range[1] - _range[0]) + _range[0]

    def _scale_to_range_flipped(self, arr, _range):
        # normalkized to 0,1
        arr = (arr - np.min(arr)) / (np.max(arr) - np.min(arr))
        return (1 - arr) * (_range[1] - _range[0]) + _range[0]

    def _get_keyword_filenumbers(self, keywords, fname, fdir=""):
...
class _mm_backend_util(_mm_base_util):
            ...
            for l in f:
                ...
                if len(line) > 0 and line[0] != "#":
                    ...
                    # otherwise, default case (add that string to the dictionary)
                        ...
                        line = line.split(": ")
                        self.params[line[0].lstrip().rstrip()] = (
                            line[1].lstrip().rstrip()
                        )

            # add filled dictionaries in their respective places
            self.params["parameters"] = dict(
                [(p, parameters[p]) for p in parameters if p is not None]
            )
                ...
                if type(self.params[required_parameter]) == dict:
                    for subparam in self.params[required_parameter]:
                        if type(self.params[required_parameter][subparam]) == dict:
                            ...
                            for subsubparam in self.params[required_parameter][
                                subparam
                            ]:
                                self.log.debug(
                                    "    - {}: {}".format(
                                        subsubparam,
                                        self.params[required_parameter][subparam][
                                            subsubparam
                                        ],
                                    )
                                )
                            ...
                            self.log.debug(
                                "  - {}: {}".format(
                                    subparam, self.params[required_parameter][subparam]
                                )
                            )
                else:
    ...
    def _get_parameter_dict(self, line):
        og = line
        line = line.split(": ")
        ...
        try:
            ...
            for value in line:
                ...
                if value[0] in ["lha_id", "morphing_max_power"]:
                    ...
                    value[1] = tuple(
                        [float(v) for v in value[1].lstrip("(").rstrip(")").split(",")]
                    )
            ...
            self.log.error(
                "incorrect parameter formatting for parameter {}".format(name)
            )
    ...
    def _get_benchmark_dict(self, string):
        string = string.split(": ")
        name = string[0].lstrip("benchmark ")
        tags = dict(
            [
                tuple((elt.split("=")[0], float(elt.split("=")[1])))
                for elt in string[1].rstrip().lstrip().split(", ")
            ]
        )
        return name, tags

    def _get_obseravble_dict(self, string):
        string = string.split(": ")
        name = string[0].lstrip("observable ")
        return name, string[1]

    def _check_valid_backend(self):
        ...
        for param in self.required_params:
            if param not in self.params:
                self.log.error(
                    "Provided backend file does not include the required key '{}'".format(
                        param
                    )
                )
                valid_flag = False
        ...
        for exp_param in self.params["parameters"]:
            for req_exp_param in self.required_experimental_params:
                if req_exp_param not in self.params["parameters"][exp_param]:
                    self.log.error(
                        "Required experimental parameter '{}' not in parameter '{}'".format(
                            req_exp_param, exp_param
                        )
                    )
                    valid_flag = False
        ...
        for benchmark in self.params["benchmarks"]:
            for existing_parameter in self.params["parameters"]:
                if existing_parameter not in self.params["benchmarks"][benchmark]:
                    self.log.error(
                        "Provided sample benchmark '{}' does not contain a value for parameter '{}'.".format(
                            self.params["benchmarks"][benchmark], existing_parameter
                        )
                    )
                    valid_flag = False
...
class _mm_simulate_util(_mm_base_util):
    ...
    def _equal_sample_sizes(self, samples, sample_limit):
    ...
    def _number_of_cards(self, samples, sample_limit):
    ...
    def _check_valid_morphing(self):
        if (
            ...
            self.log.error(
                "More or less than one 'madminer_<name>.h5' card in directory"
            )
    ...
    def _check_valid_mg5_scripts(self, samples):
        ...
        if size < 0:
            self.log.error(
                "mg_processes/signal/madminer/scripts directory does not exist here."
            )
            return self.error_codes.NoScriptError
        if size != expected:
            self.log.error(
                "Found {}/{} expected mg5 scripts. Incorrect mg5 setup.".format(
                    size, expected
                )
            )
...
class _mm_train_util(_mm_base_util):
    ...
    def _get_raw_mg5_arrays(self, include_array=None):
        ...
        return (
            mg5_obs,
            mg5_weights[include_array],
            mg5_norm_weights[include_array],
            n_mg5,
        )

    def _get_automatic_ranges(self, obs, weights):

        return np.mean(
            np.array(
                [
                    [
                        np.histogram(obs[:, i], weights=weights[j])[1][[0, -1]]
    ...
    def _get_mg5_and_augmented_arrays(
        ...
        aug_values, aug_bins = [
            np.asarray([[subarr for subarr in arr] for arr in x_aug[:, :, i]])
            for i in range(2)
        ]
        mg5_values, mg5_bins = [
            np.asarray([[subarr for subarr in arr] for arr in x_mg5[:, :, i]])
            for i in range(2)
        ]
        ...
        digitized = (
            np.asarray(
                [
                    np.digitize(
                        mg5_obs[:, i],
                        np.histogram(mg5_obs[:, i], bins=bins[i], range=ranges[i])[1],
                    )
            ...
            [
                [
                    [
                        np.sqrt(
                            np.sum(
                                weight[digitized[bn] == b] * weight[digitized[bn] == b]
                            )
                        )
                ...
                for bn, bin_n in enumerate(bins)
            ]
        )
        scale_facs = 1.0 / np.asarray(
            [
                [
                    np.max(
                        np.histogram(
                    ...
                    / np.max(mg5_values[i, j])
                    for j, weight in enumerate(mg5_norm_weights)
                ]
                for i, obs in enumerate(observables)
            ]
        )
        mg5_err_scaled = (np.vstack(mg5_err).T * np.hstack(scale_facs)).T.reshape(
            mg5_err.shape
        )
        ...
        return (
            [self.error_codes.Success],
            (aug_values, aug_bins, n_aug),
            (mg5_values, mg5_bins, n_mg5, mg5_err_scaled),
        )

    def _compare_mg5_and_augmented_data(self, x_aug, x_mg5, y_fac=1.0, threshold=2.0):

        r = abs(x_mg5[0] - x_aug[0]) / x_mg5[3]

        pers = [
            [len(elt[np.where(elt >= threshold)]) / len(elt) for elt in relt]
            for relt in r
        ]

        return r, pers

    def _check_valid_training_data(self):
        size = self._dir_size(
            pathname=self.dir + "/mg_processes/signal/Events", matching_pattern="run_"
        )
        if size < 0:
    ...
    def _check_valid_augmented_data(self, sample_name, expected_benchmarks=None):
        size = self._dir_size(
        ...
        if size < 0:
            self.log.error(
                "data/samples/{} directory does not exist!".format(sample_name)
            )
            self.log.error("Augmented data not parsed (or detected)")
            return self.error_codes.NoDirectoryError
        elif size == 0:
            self.log.error(
                "data/samples/{} directory does not contain any files with the given sample_name".format(
                    sample_name
                )
            )
            self.log.error("Augmented data not parsed (or detected)")
        ...
        if expected_benchmarks is not None:
            if size != expected_benchmarks:
                self.log.error(
                    "data/samples/{} directory contains an incorrect # of files with given sample_name ".format(
                        sample_name
                    )
                )
                self.log.error(
                    " - {}/{} expected files".format(size, expected_benchmarks)
                )
    ...
    def _check_for_matching_augmented_data(self, sample_size, sample_benchmark):
        farr = [
            (f, self._load_config(f))
            for f in glob.glob(
                "{}/evaluations/*/*/augmented_sample.mmconfig".format(self.dir)
            )
            + glob.glob("{}/data/samples/*/augmented_sample.mmconfig".format(self.dir))
        ]
        for path, config_dict in farr:
            if (
                config_dict["augmentation_samples"] == sample_size
                and config_dict["augmentation_benchmark"] == sample_benchmark
            ):
                return path
        return None

    def _check_valid_trained_models(self, training_name, sample_name="*"):
        ...
        if size == 0:
            self.log.error(
                "no models detected with training name {}".format(training_name)
            )
            return self.error_codes.NoTrainedModelsError

        if size > 1:
            self.log.error(
                "Multiple models with name matching '{}'. Please specify further among the following possibilities:".format(
                    training_name
                )
            )
            for fname in matching_files:
                self.log.error(
                    " - Model '{}', trained on sample '{}'".format(
                        "_".join(fname.split("/")[-1].split("_")[:-1]),
                        "_".join(fname.split("/")[-2].split("_")[2:]),
                    )
                )
    ...
    def _check_valid_evaluation_data(self, evaluation_name, training_name="*"):
        matching = glob.glob(
            "{}/evaluations/{}/{}".format(self.dir, training_name, evaluation_name)
        )
        size = len(matching)

        if size == 0:
            self.log.error(
                "no evaluation instances with name {}".format(evaluation_name)
            )
            return self.error_codes.NoEvaluatedModelError
        if size > 1:
            self.log.error(
                "More than one trained model with evaluation name '{}'".format(
                    evaluation_name
                )
            )
            self.log.error(
                "Please specify training model along with eval name, from the following:"
            )
    ...
    def _get_evaluation_path(self, evaluation_name, training_name="*"):
...
class _mm_util(_mm_backend_util, _mm_simulate_util, _mm_train_util, _mm_base_util):
    ...
    def _submit_condor(self, arg_list, max_runtime=30.0):
        """
        ...
        with open("{}/condor/core.sub".format(self.dir), "w+") as f:
            ...
            f.write(
                "arguments = -m mm_scripting_util.run {}\n".format(" ".join(arg_list))
            )
            f.write(
                "output = {}/condor/output.$(ClusterId).$(ProcId).out\n".format(
                    self.dir
                )
            )
            f.write(
                "error = {}/condor/error.$(ClusterId).$(ProcId).err\n".format(self.dir)
            )
